...
The system SHALL support dragging visible handle elements on the left or right edge of a task bar to change its start date or end date independently.

#### Scenario: Drag left edge changes start date
- **WHEN** a user drags the left handle of a bar 60px left (2 days at 30px/day)
...
- **WHEN** a user drags the right handle of a bar 30px right
- **THEN** the task's `endDate` SHALL increase by 1 day and `startDate` SHALL remain unchanged

#### Scenario: Edge hit area
- **WHEN** the pointer is within 8px of the left or right edge of a task bar (including the 4px visible handle + 4px invisible margin)

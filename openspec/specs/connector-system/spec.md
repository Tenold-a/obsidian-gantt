...
Define the interface and data contracts for connector scripts that fetch, transform, and optionally push task/project/person data between external systems and the Gantt chart.
## Requirements
...
- **WHEN** a connector's `push` method completes successfully
- **THEN** the return value SHALL include `success: true`

#### Scenario: Push result indicates failure with error message
- **WHEN** a connector's `push` method fails
- **THEN** the return value SHALL include `success: false` and an `error` string describing the failure

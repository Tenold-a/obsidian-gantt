...
The `parseCSV` function SHALL handle UTF-8 BOM markers.
...
- **WHEN** `parseCSV` is called with a quoted field containing leading/trailing spaces
- **THEN** the spaces inside quotes SHALL be preserved in the value

### Requirement: CsvParseOptions type definition
The `CsvParseOptions` interface SHALL be exported from `gantt-core` with an optional `delimiter` field defaulting to `','`.

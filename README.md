...
- **任务链接编辑** —— 支持编辑任务的外部链接 URL，保存后正确持久化
...
### 项目详情字段
- **描述** —— 项目的富文本备注，支持 Markdown 和 HTML 渲染
- **描述查看器** —— 自动折叠连续空行，最大高度 200px 内滚动，点击「查看完整描述」弹窗展示全文
- **需求方** —— 干系人或部门名称
...
### 数据容错与兜底
- **日期校验** —— `isValidDate()` 验证 YYYY-MM-DD 格式合法性，非法日期返回 0 像素而非 NaN
- **缺失日期处理** —— 无开始日期的任务自动跳过渲染；无结束日期的任务以开始日期作为结束日期
- **日期倒置修复** —— 结束日期早于开始日期时自动交换，确保时间条始终正向
- **缺失字段保护** —— 缺少 id、title 等必填字段时使用默认值填充，不会崩溃
- **项目颜色兜底** —— 无颜色字段时基于项目 ID 哈希分配 8 色调色板中的稳定颜色
- **缺失实体过滤** —— 无 id 的项目、人员、任务在数据处理阶段自动过滤，不进入渲染
